-- ============================================================================
-- SCHOLARFLOW / EDUTRACK ERP — COMPLETE FINAL DATABASE MIGRATION
...
ALTER TABLE public.user_profiles ADD COLUMN IF NOT EXISTS must_reset_password BOOLEAN DEFAULT false;
ALTER TABLE public.user_profiles ADD COLUMN IF NOT EXISTS failed_login_attempts INT DEFAULT 0;
ALTER TABLE public.user_profiles ADD COLUMN IF NOT EXISTS locked_until TIMESTAMPTZ DEFAULT NULL;
...
-- CLASSES TABLE (with teacher_id from the start)
...
-- Add teacher_id to classes if table already existed without it
ALTER TABLE public.classes ADD COLUMN IF NOT EXISTS teacher_id UUID REFERENCES public.user_profiles(id) ON DELETE SET NULL;

-- SUBJECTS TABLE (with teacher_id and class_id from the start)
...
-- Add missing columns to subjects if table already existed
ALTER TABLE public.subjects ADD COLUMN IF NOT EXISTS class_id UUID REFERENCES public.classes(id) ON DELETE SET NULL;
ALTER TABLE public.subjects ADD COLUMN IF NOT EXISTS teacher_id UUID REFERENCES public.user_profiles(id) ON DELETE SET NULL;
...
--------------------------------------------------------------------------------
-- 4. AUTH SYNC TRIGGER (Auto-sync Supabase Auth users → user_profiles)
--------------------------------------------------------------------------------

-- Function: when a new user is created in auth.users, create their profile
...
BEGIN
  ...
  INSERT INTO public.user_profiles (id, email, first_name, last_name, roles)
  VALUES (
    NEW.id,
    NEW.email,
    v_first,
...
--------------------------------------------------------------------------------
-- 5. BACKFILL: Sync existing auth.users that have no user_profiles row
--------------------------------------------------------------------------------
...
SELECT
  ...
  ARRAY[UPPER(COALESCE(au.raw_user_meta_data->>'role', 'STUDENT'))]
FROM auth.users au
WHERE NOT EXISTS (
...
-- Fix any existing profiles that have wrong/missing role based on metadata
UPDATE public.user_profiles up
SET roles = ARRAY[UPPER(COALESCE(au.raw_user_meta_data->>'role', 'STUDENT'))]
FROM auth.users au
WHERE up.id = au.id
  AND (up.roles = ARRAY['STUDENT'] OR up.roles IS NULL)
  AND au.raw_user_meta_data->>'role' IS NOT NULL
  AND UPPER(au.raw_user_meta_data->>'role') != 'STUDENT';

--------------------------------------------------------------------------------
-- 6. ROW LEVEL SECURITY POLICIES (Full permissive for authenticated)
--------------------------------------------------------------------------------
...
-- Done
SELECT 'Migration complete! All tables, columns, triggers and policies are set up.' AS status;
